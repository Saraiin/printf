	...
	int flags, int width, int precision, int size)
{
	...
	return (write_mem_addr(buffer, id, length,
		width, flags, pd, ec, pd_start));
}
	...
	int flags, int width, int precision, int size)
{
	...
	while (str[i] != '\0')
	{
		if (ckprintable(str[i]))
			buffer[i + offset] = str[i];
		else
			offset += aphexa_code(str[i], buffer, i + offset);
...
/************************* PRINT REVERSE *************************/
/**
 * pr_reverse - Prints reverse string.
 * @types: Lista of arguments
 * @buffer: Buffer array to handle print
 * @flags:  Calculates active flags
 * @width: get width
...
/**
 * pr_rot13string - Print a string in rot13.
 * @types: Lista of arguments
 * @buffer: Buffer array to handle print
 * @flags:  Calculates active flags
 * @width: get width
